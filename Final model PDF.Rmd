...
To improve accuracy beyond Prophet’s default capabilities, we build a hybrid model:
...
- Extracting the last 12 months of Prophet’s future forecast
...
We now evaluate the hybrid model’s forecast accuracy by:
...
- AIC (Akaike Information Criterion) — prefers better fit, less strict
- BIC (Bayesian Information Criterion) — prefers simpler models, penalizes complexity more
...
In this final modeling step, we leverage the volatility estimates (σ<sub>t</sub>) from the fitted GARCH model to explain and correct residuals from our hybrid Prophet + ARIMA model.

### Workflow:
1. Extract conditional standard deviations from the GARCH model
2. Fit a linear regression: residual ~ volatility (GARCH σ<sub>t</sub>)
...
# ---------------------------------------
# Note that this requires refitting the data with a train set, and testing against the test set. That is done on line  # 223 of the file (preparing prophet data). Only use the code below to test split data. If the model is fit with all   # the data, do not use the code below.
